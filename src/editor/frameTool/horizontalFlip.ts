...
export class HorizontalFlip implements FrameTool {
    use(editor: Editor) {
        let canvas = editor.getCurrentCanvas();
        let layersLength = canvas.getLayersLength();
        let size = canvas.getSize();

        // get the columns at the left half and set them to the right half and vice versa
            ...
            if (currentLayer != null) {
                ...
                for (let x = 0; x < size.width; x++) {
                    for (let y = 0; y < size.height; y++) {
                        let point = { x, y };
                        let newPoint = { x, y: size.height - 1 - y };
